...
# Make a factor?
sunset_data$Good.Sunset <- factor(sunset_data$Good.Sunset, labels = c("Yes", "No"))
...
sunset_recipe <- recipe(Good.Sunset ~ ., data=sunset_train) %>%
  step_rm(State.Name, datetime, sunset, conditions, description, icon) %>%
