...
Before these autoplots, however, I think it would be interesting to take a look at which predictor variables were most useful in predicting our response variable, `good_sunset`. To my surprise, the Nitrogen Dioxide levels in the air was the most useful variable in predicting how good a sunset will be. Hey, the more you know!
...
The ROC AUC scores seem to vary quite a lot depending on the number of trees, but overall it is clear that more trees leads to a higher ROC AUC score. The optimal node size was 4, present in the middle plot, with 7 trees, and 6 randomly selected predictors. Unless our next model can surpass this, the random forest seems to be the best model.
...
As we can see in our tibble, the Random Forest model performed the best overall with a ROC AUC score of 0.9972! Of course, this is only fitted on the training data, so our model still needs to perform on our testing data that we have reserved for exactly this. Since we will be moving forward with this Random Forest model, let's now find its true performance on our testing dataset.

# Best Model Results

Since our random forest model had the best overall performance, we now want to analyze how strong it is on data it has not seen yet. The high ROC AUC scores you saw above were the models ability to predict a sunset's quality using the same data it was originally trained on, thus explaining its strong results. However, before we discover its true ability, we will explore more what a random forest model is and how it works.
...
Our random forest model was able to predict beautiful sunsets in our testing data with about 86% accuracy. That's awesome! I can now confidently tell my friends whether or not a sunset is really worth dropping everything to watch, with about 86% accuracy.
...
# Extension: Exploring the Support Vector Machine Model

Before we conclude our project, let's take a quick look at how our support vector machine model performed on our testing data. While it did not have best ROC AUC performance out of our seven models, the support vector machine algorithm is still incredibly powerful and worth exploring further to discover its true results.     
...
sunset_svm_accuracy <- augment(svm_final_fit, new_data = sunset_test, type = 'prob') %>%
  accuracy(good_sunset, .pred_class) %>%
  select(.estimate)
```

The Support Vector Machine ROC AUC score on the testing data:
```{r}
sunset_svm_roc_auc
```
