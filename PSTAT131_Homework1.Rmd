...
Supervised learning is a branch of machine learning in data science that allows for accurate prediction of future responses given certain known variables, allows for users to directly understand how these known predictors affect results, as well as allows users to find the most accurate model and determine the precision of their estimations.
Unsupervised learning has "no answer key" (*from 'Lecture: Course Overview and Introduction'*). There is no way to check how well the model did, or how accurate it was. The user is unable to check the results of their estimations or predictions, and therefore cannot judge the accuracy of their model.
...
The bias-variance tradeoff is related to the use of mechanistic or empirically-driven models in a variety of ways. In the case of a mechanistic model, since predictions are being made based on theory, there will be higher bias and less variance since certain results will already be expected. In the case of an empirically-driven model, if the training data is strictly followed based on observed events, there will be high variance but much less bias. In either of these cases, the best solution is to find the optimal balance between bias and variance in order to get a good tradeoff.
...
hist(mpg$hwy)
```
The histogram of highway miles per gallon seems so be slightly skewed right with a majority of cars having a highway mileage between 25 mpg and 30 mpg.

## Question 2

```{r}
plot(mpg$hwy, mpg$cty, main="Highway Mileage vs City Mileage", xlab="Highway MPG",
     ylab="City MPG", axes=TRUE)
```
